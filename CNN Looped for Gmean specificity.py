...
def weighted_mse(y_pred, y_true):
    ...
    minority_weight=0.005
        # Apply the weights
    loss = K.mean(K.square(y_pred - y_true)*((y_true * majority_weight) + (1. - y_true) * 
                           minority_weight), axis=-1)
    ...
    # Generates Abnormal data points
        for i in range(0,b):
            ...
            if abtype == 5:
                ...
                Data2 = np.concatenate((Data2,y), axis = 0)
            if abtype == 6:
                y = np.random.normal(mu,sigma,w)+t*((-1)^s)   # Stratification
                Data2 = np.concatenate((Data2,y), axis = 0)
...
#                if abtype == 2:
#                        y_new = np.random.normal(mu,sigma,w) - t*s        # Down trend(+)
#                        Data_ext = np.concatenate((Data_ext,y_new), axis = 0)            
#                if abtype == 3:
...
#        x_ext = Data_ext.iloc[:,:-1]
        ...
        X_train, X_test, Y_train, Y_test = train_test_split(Data, Response_raw, 
                                                            test_size=0.33)
...
#        Y_test = np.vstack([Y_test, y_ext])
        ...
        model.fit(X_train, Y_train, batch_size=None, epochs=5, verbose=0, 
                  class_weight=None, callbacks=[reduce_lr]) scores = model.evaluate(X_test, Y_test, batch_size=16)
...
Specificity_array=np.reshape(Specificity_array,(10,1))
Sensitivity_array=np.reshape(Sensitivity_array,(10,1))
Gmean_array=np.reshape(Gmean_array,(10,1))
print(Gmean_array)

Gmean_average = np.mean(Gmean_array)
print(Gmean_average, "Gmean average")
Gmean_sd = np.std(Gmean_array)
print(Gmean_sd, "Gmean sd")

Specificity_average = np.mean(Specificity_array)
print(Specificity_average, "Specificity average")
Specificity_sd = np.std(Specificity_array)
print(Specificity_sd, "Specificity sd")

Sensitivity_average = np.mean(Sensitivity_array)
print(Sensitivity_average, "Sensitivity average")
Sensitivity_sd = np.std(Sensitivity_array)
print(Sensitivity_sd, "Sensitivity sd")
