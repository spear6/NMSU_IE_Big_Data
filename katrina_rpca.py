...
ma=prediction.rolling(4).mean()
plt.plot(pred_vec,linestyle="",marker="o", label='Predicted')
plt.plot(true_vec, linestyle="",marker=".",color="g", label='True')
plt.plot(ma,linestyle="solid",marker="",label="MRPCA-LSTM")
...
prediction=pred_vec
prediction=pd.DataFrame(prediction)
ma=prediction.rolling(10).mean()
mstd=prediction.rolling(10).std()

min=np.maximum(0,np.array(ma-1*mstd))
max=np.array(ma+1*mstd)

plt.title('Predicted Response (Demand) with $95\%$ Confidence Intervals' )
plt.xlabel('Observation')
plt.ylabel('Prediction')
plt.plot(prediction.index, prediction,'k',label="Model Predictions",linewidth=0.25)
#plt.plot(ma.index,ma,'b',label="Smoothed Prediction")
plt.plot(min,label="Lower 95% CI", linewidth=2)
plt.plot(max,label="Upper 95% CI",linewidth=2)
#plt.fill_between(ma.index,min,max)
plt.legend()

plt.plot(pred_vec, label="Model Predicted Demand")
plt.plot(true_vec,label="Actual Demand")
plt.title('Actual versus Predicted Fuel Demand for Hurricane Katrina' )
plt.xlabel('Observation')
plt.ylabel('Prediction')
plt.legend()
plt.show()
...
plt.xlabel('Observation')
plt.ylabel('Scaled Prediction Variables')
plt.yticks(np.arange(8), ("SD","QR","QA","SP","PP","IP","IQ","EQ"))
plt.ylim(0, 8.0)
plt.show()
